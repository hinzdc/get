...
$downloadPath = "$env:TEMP\removaltool.exe"
# Pastikan folder tujuan ada
$downloadPathFolder = [System.IO.Path]::GetDirectoryName($downloadPath)
if (!(Test-Path -Path $downloadPathFolder)) {
    New-Item -ItemType Directory -Path $downloadPathFolder -Force | Out-Null
}
    ...
    if ($bitsService) {
        if ($bitsService.Status -ne "Running") {
            ...
            try {
                ...
                Write-Host " X Failed to start BITS service. Falling back to Invoke-WebRequest." -ForegroundColor Red
                $useBits = $false
            }
        }
    } else {
        Write-Host " X BITS service is not available on this system. Using Invoke-WebRequest instead." -ForegroundColor Red
        $useBits = $false
    }

    try {
        if ($useBits) {
            Write-Host " + Downloading $name using BitsTransfer." -ForegroundColor Cyan
            ...
            Write-Host " + Downloading $name using Invoke-WebRequest..." -ForegroundColor Cyan
